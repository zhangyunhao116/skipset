...
type {{.StructPrefix}}Set{{.StructSuffix}}{{.TypeParam}} struct {
	header       *{{.StructPrefixLow}}node{{.StructSuffix}}[T]
	length       int64
...
func new{{.StructPrefix}}Node{{.StructSuffix}}{{.TypeParam}}(value T, level int) *{{.StructPrefixLow}}node{{.StructSuffix}}[T] {
	n := &{{.StructPrefixLow}}node{{.StructSuffix}}[T]{
		value: value,
		level: uint32(level),
	}
	if level > op1 {
...
func (n *{{.StructPrefixLow}}node{{.StructSuffix}}[T]) loadNext(i int) *{{.StructPrefixLow}}node{{.StructSuffix}}[T] {
	return (*{{.StructPrefixLow}}node{{.StructSuffix}}[T])(n.next.load(i))
}

func (n *{{.StructPrefixLow}}node{{.StructSuffix}}[T]) storeNext(i int, next *{{.StructPrefixLow}}node{{.StructSuffix}}[T]) {
	n.next.store(i, unsafe.Pointer(next))
}

func (n *{{.StructPrefixLow}}node{{.StructSuffix}}[T]) atomicLoadNext(i int) *{{.StructPrefixLow}}node{{.StructSuffix}}[T] {
	return (*{{.StructPrefixLow}}node{{.StructSuffix}}[T])(n.next.atomicLoad(i))
}

func (n *{{.StructPrefixLow}}node{{.StructSuffix}}[T]) atomicStoreNext(i int, next *{{.StructPrefixLow}}node{{.StructSuffix}}[T]) {
...
func (s *{{.StructPrefix}}Set{{.StructSuffix}}[T]) findNodeRemove(value T, preds *[maxLevel]*{{.StructPrefixLow}}node{{.StructSuffix}}[T], succs *[maxLevel]*{{.StructPrefixLow}}node{{.StructSuffix}}[T]) int {
...
// The returned preds and succs always satisfy preds[i] > value >= succs[i].
func (s *{{.StructPrefix}}Set{{.StructSuffix}}[T]) findNodeAdd(value T, preds *[maxLevel]*{{.StructPrefixLow}}node{{.StructSuffix}}[T], succs *[maxLevel]*{{.StructPrefixLow}}node{{.StructSuffix}}[T]) int {
	x := s.header
	for i := int(atomic.LoadUint64(&s.highestLevel)) - 1; i >= 0; i-- {
		succ := x.atomicLoadNext(i)
		for succ != nil && {{Less "succ.value" "value"}} {
...
func unlock{{.Name}}{{.TypeParam}}(preds [maxLevel]*{{.StructPrefixLow}}node{{.StructSuffix}}[T], highestLevel int) {
	var prevPred *{{.StructPrefixLow}}node{{.StructSuffix}}[T]
...
func (s *{{.StructPrefix}}Set{{.StructSuffix}}[T]) Add(value T) bool {
	level := s.randomlevel()
	var preds, succs [maxLevel]*{{.StructPrefixLow}}node{{.StructSuffix}}[T]
	for {
		...
		var (
			...
			valid                = true
			pred, succ, prevPred *{{.StructPrefixLow}}node{{.StructSuffix}}[T]
		)
...
func (s *{{.StructPrefix}}Set{{.StructSuffix}}[T]) randomlevel() int {
...
func (s *{{.StructPrefix}}Set{{.StructSuffix}}[T]) Contains(value T) bool {
	x := s.header
	for i := int(atomic.LoadUint64(&s.highestLevel)) - 1; i >= 0; i-- {
...
func (s *{{.StructPrefix}}Set{{.StructSuffix}}[T]) Remove(value T) bool {
	var (
		nodeToRemove *{{.StructPrefixLow}}node{{.StructSuffix}}[T]
		isMarked     bool // represents if this operation mark the node
		topLayer     = -1
		preds, succs [maxLevel]*{{.StructPrefixLow}}node{{.StructSuffix}}[T]
	)
	for {
...
func (s *{{.StructPrefix}}Set{{.StructSuffix}}[T]) Range(f func(value T) bool) {
...
func (s *{{.StructPrefix}}Set{{.StructSuffix}}[T]) Len() int {
